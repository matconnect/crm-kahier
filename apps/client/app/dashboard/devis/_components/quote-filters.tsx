...
export function QuoteFilters({ initialQuery = "", initialStatus = "" }: { initialQuery?: string; initialStatus?: string }) {
    ...
    React.useEffect(() => {
        const timer = window.setTimeout(() => {
            ...
            startTransition(() => {
                router.replace(`/dashboard/devis${params.size ? `?${params.toString()}` : ""}`);
            });
        }, 250);
